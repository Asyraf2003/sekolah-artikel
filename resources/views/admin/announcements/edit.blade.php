<x-page.admin>
    @php
        ...
        $eventDateValue = old('event_date', $announcement->event_date?->format('Y-m-d') ?? now()->toDateString());

        $isPublishedChecked = (bool) old('is_published', $announcement->is_published);

        $title = $announcement->title_id ?: ($announcement->title_en ?: ($announcement->title_ar ?: '—'));

        $isTrashed = method_exists($announcement, 'trashed') && $announcement->trashed();

        // Guard link_url against unsafe schemes (javascript:, data:, etc.)
        $safeHref = function (?string $href): ?string {
            $href = trim((string) $href);
            if ($href === '') return null;
            if ($href === '#') return $href;
            if (\Illuminate\Support\Str::startsWith($href, ['/'])) return $href;
            if (\Illuminate\Support\Str::startsWith($href, ['https://', 'http://'])) return $href;
            return null;
        };

        $href = $safeHref($announcement->link_url);
    @endphp

    <div class="page-heading">
        <div class="d-flex flex-wrap justify-content-between align-items-start gap-2">
            <div>
                <h3 class="mb-1">Edit Pengumuman</h3>
                <p class="text-muted mb-0">
                    Perbarui pengumuman: <span class="fw-semibold">{{ $title }}</span>
                    <span class="text-muted">#{{ $announcement->id }}</span>
                    @if($isTrashed)
            ...
            <div class="d-flex gap-2 flex-wrap">
                ...
                @if($href)
                    <a href="{{ $href }}" target="_blank" rel="noopener noreferrer" class="btn btn-outline-primary">
                        <i class="bi bi-box-arrow-up-right"></i> Preview Link
                    </a>
                @endif

                @if($isTrashed && \Illuminate\Support\Facades\Route::has('admin.announcements.restore'))
                    <form action="{{ route('admin.announcements.restore', $announcement->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="btn btn-outline-success"
                                onclick="return confirm('Pulihkan pengumuman ini?')">
                            <i class="bi bi-arrow-counterclockwise"></i> Restore
                        </button>
                    </form>
                @endif

                <button form="announcementEditForm" type="submit" class="btn btn-primary">
        ...
        <section class="section">
            ...
            @if($isTrashed)
                <div class="alert alert-secondary" role="alert">
                    ...
                    Pengumuman ini sedang berstatus <strong>Deleted</strong>. Anda masih bisa mengedit, tapi biasanya lebih waras direstore dulu.
                </div>
            @endif
                            ...
                            <div class="card-body">
                                    ...
                                    <div class="tab-pane fade show active" id="pane-id" role="tabpanel" aria-labelledby="tab-id">
                                        <div class="mb-3">
                                            ...
                                            <input type="text"
                                                   ...
                                                   value="{{ old('title_id', $announcement->title_id) }}"
                                        ...
                                        <div class="mb-0">
                                            ...
                                            <textarea name="desc_id"
                                                      ...
                                                      placeholder="Tulis deskripsi pengumuman...">{{ old('desc_id', $announcement->desc_id) }}</textarea>
                                    ...
                                    <div class="tab-pane fade" id="pane-en" role="tabpanel" aria-labelledby="tab-en">
                                        ...
                                        <div class="mb-0">
                                            ...
                                            <textarea name="desc_en"
                                                      ...
                                                      placeholder="Optional English description...">{{ old('desc_en', $announcement->desc_en) }}</textarea>
                                    ...
                                    <div class="tab-pane fade" id="pane-ar" role="tabpanel" aria-labelledby="tab-ar">
                                        ...
                                        <div class="mb-0">
                                            ...
                                            <textarea name="desc_ar"
                                                      ...
                                                      placeholder="اختياري...">{{ old('desc_ar', $announcement->desc_ar) }}</textarea>
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <div class="form-text">Link tujuan saat tombol “Kunjungi” ditekan. (Mending http/https atau /path)</div>
                                    </div>
                                </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="small text-muted">
                                    <div class="mb-1">
                                        ...
                                        Dibuat: <strong>{{ $announcement->created_at?->format('d M Y H:i') ?? '—' }}</strong>
                                    </div>
                                    <div class="mb-0">
                                        <i class="bi bi-arrow-repeat me-1"></i>
                                        Update: <strong>{{ $announcement->updated_at?->format('d M Y H:i') ?? '—' }}</strong>
                                    </div>
                                </div>
                    ...
                    <div class="card border-danger mt-3">
                        ...
                        <div class="card-body d-flex flex-wrap justify-content-between align-items-center gap-2">
                            <div class="text-muted">
                                @if(!$isTrashed)
                                    Hapus pengumuman ini (soft delete).
                                @else
                                    Pengumuman ini sudah dihapus (soft delete).
                                @endif
                            </div>
                            ...
                            <div class="d-flex gap-2 flex-wrap">
                                ...
                                @else
                                    @if(\Illuminate\Support\Facades\Route::has('admin.announcements.restore'))
                                        <form action="{{ route('admin.announcements.restore', $announcement->id) }}"
                                              method="POST"
                                              onsubmit="return confirm('Pulihkan pengumuman ini?')">
                                            @csrf
                                            @method('PATCH')
                                    ...
                                    @if(\Illuminate\Support\Facades\Route::has('admin.announcements.forceDestroy'))
                                        <form action="{{ route('admin.announcements.forceDestroy', $announcement->id) }}"
                                              method="POST"
                                              onsubmit="return confirm('Hapus permanen pengumuman ini? Ini tidak bisa dibatalkan.')">
                                            @csrf
                                            @method('DELETE')
    ...
    <script>
        function togglePublishedAt() {
            ...
            // kalau user matiin publish, kosongin jadwal biar ga nyangkut
